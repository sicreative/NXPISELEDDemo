...
#endif
{
	...
	*count = 0u;
	...
	while ((*count < limit) && (err == kUARTNoError)) {
		...
		err = LPUART_DRV_ReceiveDataBlocking(INST_LPUART1,buffer,1u,UART_TIMEOUT);
#if __PPC_EABI__
...
#endif
{
	...
	*/
	...
	if (LPUART_DRV_SendDataBlocking(INST_LPUART1,buffer,(uint32_t)*count,UART_TIMEOUT) != kUARTNoError) {
		*count = 0u;
		MISRA_EXCEPTION_RULE_14_7()
		return (int_t)__io_error;
	}
...
static UARTError __init_uart_console(void)
{
	...
	if (initialized == 0) {
		err =  LPUART_DRV_Init(INST_LPUART1,&lpuart1_State,&lpuart1_InitConfig0);
